...
local Keys = loadstring(game:HttpGet("https://raw.githubusercontent.com/Aspect6666/roblox/main/AlphanumericKeys.lua"))()
...
local theme = AspectThemes.Aspect

-- Functions
local old_err = error
local function error(message)
    old_err("[AspectUILib] "..tostring(message))
end
...
function library:SetTheme(themeSel)
    if AspectThemes[themeSel] then 
        theme = AspectThemes[themeSel]
    elseif typeof(themeSel) == "table" then
        for i,v in pairs(AspectThemes.Aspect) do
...
function library:CreateMain(title, description, keycode)
    ...
    if getgenv().AspectUILib then 
        getgenv().AspectUILib:Destroy() 
        onSelfDestroy()
    end
    local firstTab
    local AspectUI = Instance.new("ScreenGui")
    if syn then 
        syn.protect_gui(AspectUI)
    end
    AspectUI.Parent = gethui and gethui() or game.CoreGui
    getgenv().AspectUILib = AspectUI
    closeconnection = UIS.InputEnded:Connect(function(input,yes)
        ...
        if input.KeyCode == library["Bind"] and not yes and not library["IsBinding"] then
            AspectUI.Enabled = not AspectUI.Enabled
        end
    end)
    ...
    AspectUI.Name = "AspectUI"
    AspectUI.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

    Main.Name = "Main"
    Main.Parent = AspectUI
            ...
            function library4:SetRainbow(val)
                ...
                if library4["RainbowValue"] then
                    ...
                    heh = coroutine.resume(coroutine.create(function()
                        ...
                        until library4["RainbowValue"] == false or getgenv().AspectUILib == nil
                    end))
                end
            end
    ...
    function library2:CreateNotification(title, description, callback)
        callback = callback or function() end
        if AspectUI:FindFirstChild("Notification") then 
            AspectUI:FindFirstChild("Notification"):Destroy()
        end
